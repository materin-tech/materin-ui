...
## 0.6.0 — 2026-09
加上**数据展示层（一）**，组件数 44 → **55**。

- 新增 11 个：
  - Progress（线性/环形、四种状态色、不确定进度转条）
  - Statistic（千分位、小数位、前后缀、成功/危险色）
  - Descriptions（横竖两向、可带边框）、Result（四种状态图标 + 操作区）
  - Collapse + CollapseItem（多开或手风琴、受控可用）
  - Timeline + TimelineItem（节点四色、时间戳、末节点自动收线）
  - Pagination（省略号页码、受控、双事件）、Segmented（互斥选项、可撑满）
